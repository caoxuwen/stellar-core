    ...
    for (auto const& tradingPair : app.getConfig().TRADING)
    {
...

        {
            ...
            for (auto& trustline : trustlines)
            {
                ...
                if (tl1.debt > 0)
                {
                    ...
                    // tl.debt == coin1.debt
                    if (!applyCreateLiquidationOffer(app, ls, lh, tl.accountID,
                                                     coin2, coin1, price,
                                                     -tl2.debt, false))
                    {
                        app.getMetrics()
                            .NewMeter(
                                {"op-liquidation", "failure", "process-error"},
                                "operation")
                            .Mark();
                        innerResult().code(LIQUIDATION_PROCESS_ERROR);
                        return false;
                    }
                }
                else if (tl2.debt > 0)
...

        {
            ...
            for (auto& trustline : trustlines)
            {
                ...
                // also cancel all account offers
                if (!applyCreateLiquidationOffer(app, ls, lh, tl.accountID,
                                                 coin1, coin2, Price(1, 1), 0,
                                                 true))
                {
                    app.getMetrics()
                        .NewMeter(
